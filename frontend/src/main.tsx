...
function App() {
  const [root, setRoot] = useState("/root/rag_learning");
  ...
  async function previewIndex() {
    ...
    setState(
      result.ok
        ? { status: "preview", data: result.data }
  ...
  async function buildIndex() {
    setState({
      status: "loading",
      label: "正在全量构建 SQLite + FAISS 持久化索引...",
    });
  ...
  async function runRetrieval() {
    ...
    setState({
      status: "loading",
      label:
        needsEmbedding && provider === "bge-m3"
          ? "正在加载 bge-m3 并执行混合检索..."
          : "正在检索知识库...",
    });
    const result = await postJson<VectorSearchResponse>(
      `/api/retrieval/${mode}`,
      {
        root,
        query,
      ...
      <main className="shell">
        ...
        <section className="hero">
          <div className="hero-copy">
            ...
            <h1>先审计证据，再相信回答。</h1>
            <p className="lede">
              一个面向学习和排错的知识库工作台：扫描 Markdown、观察
              chunk、运行关键词/向量/混合检索，并直接核对后端返回的原文证据。
            </p>
          ...
          <aside className="status-card" aria-label="当前运行配置">
            ...
            <p>
              {provider === "bge-m3"
                ? "本地 bge-m3 向量模型，适合真实语义检索。"
                : "hash 测试向量，无模型依赖，适合快速联调。"}
            </p>
        ...
        <section className="control-grid" aria-label="检索控制台">
          <div className="control-intro">
            ...
            <h2>配置一次可复现的检索实验</h2>
            <p>
              优先预览切分，再运行检索。这样可以区分“没召回”和“文档切分不合理”。
            </p>
          </div>
          ...
          <div className="field-group compact-fields">
            ...
            <div className="field checkbox-field">
              ...
              <label>
                <input
                  type="checkbox"
                  checked={usePersistentIndex}
                  onChange={(event) =>
                    setUsePersistentIndex(event.target.checked)
                  }
                />
          ...
          <div className="actions">
            ...
            <button
              ...
              className="ghost"
              disabled
              title="下一阶段：只处理新增、修改和删除的 Markdown 文件"
            >
              增量刷新（预留）
            </button>
            <button
              ...
              className="ghost"
              disabled
              title="下一阶段：后台定时 refresh 索引"
            >
              定时索引（预留）
            </button>
            <button type="button" onClick={runRetrieval} disabled={isLoading}>
              运行检索
            </button>
            <button
              type="button"
    ...
    return (
      <section className="panel">
        <PanelHeader
          ...
          chunkCount={state.data.chunk_count}
        />
  ...
  if (state.status === "answer") {
    return (
      <section className="panel">
        <PanelHeader
          title="RAG 回答"
          documentCount={state.data.citations.length}
          chunkCount={state.data.retrieved_chunks.length}
        />
...
function EmptyPanel() {
  return (
    <section className="panel empty">
      ...
      <p>
        用 <b>构建持久化索引</b> 写入 SQLite +
        FAISS；后续每次检索只向量化问题并查询现有索引。
      </p>
    </section>
  );
}
  ...
  return (
    <div className="trace">
      ...
      {data.keyword_results ? (
        ...
        >
          <ChunkList
            chunks={filterDisplayScores(data.keyword_results)}
            showScores
            compact
          />
        </TraceStep>
      ...
      {data.vector_results || data.debug ? (
        ...
        >
          {data.debug ? (
            <div className="debug-strip">
              <span>
                向量模型：<b>{data.debug.provider}</b>
              </span>
              <span>
                参与检索的文本块：<b>{data.debug.indexed_chunk_count}</b>
              </span>
              <span>
                问题向量维度：<b>{data.debug.query_vector_dimensions}</b>
              </span>
            </div>
          ) : null}
          {data.vector_results ? (
            <ChunkList
              chunks={filterDisplayScores(data.vector_results)}
              showScores
              compact
            />
          ) : null}
      ...
      <TraceStep
        ...
        description={
          run.mode === "hybrid"
            ? "归一化关键词分数和向量分数后加权合并，当前更偏向关键词信号，适合你的技术文档检索。"
            : "当前模式直接使用该检索器的排序结果。"
        }
      >
      ...
      <section className="trace-step">
        <header>
          ...
          <div>
            <h3>引用来源</h3>
            <p>
              这些来源是发送给模型的检索证据，回答里应使用对应的 [来源编号]。
            </p>
          </div>
        </header>
    ...
    <div className="citation-list">
      {citations.map((citation) => (
        <article className="citation" key={citation.source_id}>
          ...
          <div>
            <strong>
              {citation.document_path}:{citation.start_line}-{citation.end_line}
            </strong>
...
function ChunkList({
  ...
  chunks: Array<ChunkPreview | RetrievedChunk>;
  showScores?: boolean;
  compact?: boolean;
}) {
  if (chunks.length === 0) {
    return (
      <p className="muted">
        {showScores
          ? `没有 score ≥ ${MIN_DISPLAY_SCORE} 的文本块。`
          : "当前问题没有命中相关文本块。"}
      </p>
    );
  }
  return (
    <div className={compact ? "chunk-list compact" : "chunk-list"}>
      {chunks.map((chunk) => (
        <article
          className="chunk"
          key={`${chunk.document_path}-${chunk.chunk_index}`}
        >
          <header>
            <div>
              <span className="rank">
            ...
            {showScores && "score" in chunk ? (
              <div
                className="score"
                aria-label={`相似度分数 ${chunk.score.toFixed(4)}`}
              >
                <strong>{chunk.score.toFixed(4)}</strong>
                <span
                  style={{
                    inlineSize: `${Math.max(4, Math.min(100, chunk.score * 100))}%`,
                  }}
                />
              </div>
            ) : null}
          ...
          <p className="source">
            {chunk.document_path}:{chunk.start_line}-{chunk.end_line}
          </p>
          <pre>{chunk.text}</pre>
        </article>
      ))}
